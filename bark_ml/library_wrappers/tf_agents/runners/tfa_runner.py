...
class TFARunner:
  def __init__(self,
    ...
    self._eval_metrics = [
      tf_metrics.AverageReturnMetric(
        buffer_size=self._params["ML"]["Runner"]["evaluation_steps", "", 25]),
  ...
  def GetInitialCollectionDriver(self):
    self._initial_collection_driver = \
      dynamic_episode_driver.DynamicEpisodeDriver(
        ...
        num_episodes=self._params["ML"]["Runner"]["initial_collection_steps", "", 1])
  ...
  def GetCollectionDriver(self):
    self._collection_driver = dynamic_episode_driver.DynamicEpisodeDriver(
      ...
      policy=self._agent._agent.collect_policy,
      observers=[self._agent._replay_buffer.add_batch],
      num_episodes=self._params["ML"]["Runner"]["collection_episodes_per_cycle", "", 1])
  ...
  def Evaluate(self):
    ...
    self._logger.info("Evaluating the agent's performance in {} episodes."
      .format(str(self._params["ML"]["Runner"]["evaluation_steps", "", 20])))
    metric_utils.eager_compute(
      self._eval_metrics,
      self._wrapped_env,
      self._agent._agent.policy,
      num_episodes=self._params["ML"]["Runner"]["evaluation_steps", "", 20])
    ...
    self._logger.info(
      ...
      .format(str(self._eval_metrics[0].result().numpy()),
              ...
              str(self._params["ML"]["Runner"]["evaluation_steps", "", 20])))
