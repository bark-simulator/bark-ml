...
def run(argv):
    ...
    model = pytorch_script_wrapper.ModelLoader(os.path.join(os.path.dirname(__file__),
                                                            "model_data/{}/online_net_script.pt".format(FLAGS.network)), action_space_size, state_space_size)
    model.LoadModel()
    ...
    agent.load_models(os.path.join(os.path.dirname(__file__), "model_data", FLAGS.network))
