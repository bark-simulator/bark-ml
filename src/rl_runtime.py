...
class RuntimeRL(Runtime):
  ...
  def reset(self, scenario=None):
    ...
    self.world = self._evaluator.reset(self.world,
                                       self.scenario.eval_agent_ids)
    self.world = self._action_wrapper.reset(self.world,
                                            self.scenario.eval_agent_ids)
    self.world = self._observer.reset(self.world,
                                      self.scenario.eval_agent_ids)
    return self._observer.observe(
      world=self.world,
      agents_to_observe=self.scenario.eval_agent_ids)

  def step(self, action):
    self.world = self._action_wrapper.action_to_behavior(world=self.world,
                                                         action=action)
    self.world.step(self.step_time)
    return self.get_nstate_reward_action_tuple(
      world=self.world,
      controlled_agents=self.scenario.eval_agent_ids)
